...
let serverDmFooters = new Map();

function guildDmFootersPath(guildId) {
  return path.join(GUILD_CONFIG_ROOT, `guild_${guildId}`, 'dm_footers.json');
}

async function loadServerDmFooters() {
  await ensureRoot();
  serverDmFooters = new Map();
  try {
    const entries = await fsp.readdir(GUILD_CONFIG_ROOT, { withFileTypes: true }).catch(() => []);
    for (const e of entries) {
      if (!e.isDirectory()) continue;
      const gid = e.name.startsWith('guild_') ? e.name.slice(6) : e.name;
      const p = guildDmFootersPath(gid);
      try {
        const obj = (await readJsonFileNullable(p)) || {};
        serverDmFooters.set(gid, obj);
      } catch (err) {
        console.error(`Error loading DM footers for guild ${gid}:`, err);
      }
    }
    console.log(`Loaded DM footers for ${serverDmFooters.size} guilds`);
  } catch (err) {
    console.error('Error loading DM footers from file:', err);
  }
}

async function saveServerDmFooters() {
  for (const [gid, obj] of serverDmFooters.entries()) {
    try {
      await ensureGuildFolder(gid);
      await writeJsonFile(guildDmFootersPath(gid), obj || {});
    } catch (err) {
      console.error(`Error saving DM footers for guild ${gid}:`, err);
    }
  }
}
async function setServerDmFooterPersist(guildId, action, footerText) {
  const current = serverDmFooters.get(guildId) || {};
  current[action] = footerText;
  serverDmFooters.set(guildId, current);
  await ensureGuildFolder(guildId);
  await writeJsonFile(guildDmFootersPath(guildId), current);
}
function getServerDmFooter(guildId, action) {
  const obj = serverDmFooters.get(String(guildId)) || {};
...
const aboutInfo = {
  ...
  author: "@capitanfunny",
  features: [
    "Case lookup, voiding, and appealing system",
    "Note, Warn, Timeout/Mute, Kick, and Ban functionality",
...
const debugInfo = {
  ping: {
    ...
    test: async (ctx) => {
      try {
        return { status: '✅ OK', details: `Latency check ready.` };
      } catch (err) {
  ...
  memberinfo: {
    desc: 'Show member stats and timestamps',
    perms: 'None',
  ...
  serverinfo: {
    desc: 'Show server info and owner',
    perms: 'None',
    usage: '!serverinfo',
    test: async (ctx) => {
      try {
  ...
  setproof: {
    desc: 'Edit or add proof to an existing moderation case',
    perms: 'Manage Messages',
    usage: '!setproof <case_id> <proof>',
    test: async () => {
      return { status: '✅ OK', details: 'Handler present. Requires valid caseId + proof.' };
    }
  },
  editcase: {
    ...
    test: async () => {
      return { status: '✅ OK', details: 'Handler present. Fields: reason, proof, duration, voided, moderator, target.' };
    }
  },
  warn: {
    desc: 'Issue a warning (creates case)',
    perms: 'Manage Messages',
  ...
  role: {
    desc: 'Add or remove a role from a member (hierarchy enforced)',
    perms: 'Manage Roles',
  ...
  verification: {
    desc: 'Send verify button to configured channel',
    perms: 'Admin / Role 1404888064601624726',
    usage: '!verification',
    test: async () => {
      return { status: '✅ OK', details: 'Verification button logic present.' };
    }
  },
  updates: {
    desc: 'Post an update into a chosen channel',
    perms: 'Role 1404888064601624726',
    usage: '!updates #channel <text>',
    test: async () => {
      return { status: '✅ OK', details: 'Updates command handler present.' };
    }
  },
  membercount: {
...
function buildHelpEmbed(guildId) {
  ...
  const fields = [
    ...
    {
      name: '⚙️ set',
      value: `**Usage:** \`${currentPrefix}set [modlogs/all.logs] #channel\` or \`${currentPrefix}set dm.footer [text]\`\n**Permissions:** Administrator\n**Example:** \`${currentPrefix}set modlogs #moderation-channel\`\n**Description:** Set channels for logs or moderation DM footer`,
      inline: false
    },
    {
      name: '📎 setproof',
...
async function saveServerPrefixes() {
  try {
    const prefixesObject = Object.fromEntries(serverPrefixes);
    await fs.writeFile(SERVER_PREFIXES_FILE, JSON.stringify(prefixesObject, null, 2));
  } catch (error) {
    console.error('Error saving server prefixes to file:', error);
  }
}
...
async function saveServerLoggingChannels() {
  try {
    const channelsObject = Object.fromEntries(serverLoggingChannels);
    await fs.writeFile(SERVER_LOGGING_CHANNELS_FILE, JSON.stringify(channelsObject, null, 2));
  } catch (error) {
    console.error('Error saving server logging channels to file:', error);
  }
}
...
async function cleanupExpiredVoidedCases(guildId = null) {
  ...
  for (const caseId of caseIds) {
    ...
    if (moderationCase.guildId === guildId || (guildId === null && !moderationCase.guildId)) {
  ...
  if (cleaned > 0) {
    ...
    console.log(`Cleaned up ${cleaned} expired voided/appealed cases ${guildId ? `for guild ${guildId}` : ''}`);
  }
}

function isCaseExpired(moderationCase) {
  if (moderationCase.type === 'kick' || moderationCase.type === 'ban') {
    return false;
  }
  ...
  return moderationCase.timestamp < thirtyDaysAgo;
}
...
setInterval(() => cleanupExpiredCases(), 24 * 60 * 60 * 1000);
setInterval(() => cleanupExpiredVoidedCases(), 60 * 60 * 1000);
...
function isGuildApproved(guildId) {
  if (!process.env.APPROVED_GUILD_IDS) {
    return false;
  }
  const approvedGuilds = process.env.APPROVED_GUILD_IDS.split(',').map(id => id.trim());
  if (process.env.DEBUG === 'true') {
    console.log('Checking guild approval for:', guildId);
    console.log('Parsed approved guilds:', approvedGuilds);
    console.log('Guild approved?', approvedGuilds.includes(guildId));
  }
...
async function sendModerationDM(target, action, moderator, reason, caseId, duration = null, guildId = null) {
  try {
    ...
    if (action === 'note') {
      ...
      dmEmbed = new EmbedBuilder()
        .setColor(0xFFFFFF)
        .setTitle(`<:a_2:1415171126560165928> You have been ${action === 'mute' ? 'given a mute' : action}ed`)
        .addFields(
          { name: 'Action', value: action.toUpperCase(), inline: true },
          { name: 'Moderator', value: moderator.tag, inline: true },
          { name: 'Case ID', value: caseId, inline: true },
    ...
    if (duration) {
      dmEmbed.addFields({ name: 'Duration', value: duration, inline: true });
    }
    ...
    const customFooter = guildId ? getServerDmFooter(guildId, action) : null;
    if (customFooter && ['warn', 'kick', 'ban', 'infract'].includes(action)) {
      dmEmbed.setFooter({ text: customFooter });
    } else {
      if (action === 'note') {
        dmEmbed.setFooter({ text: 'This is a verbal warning. No further action required.' });
      } else {
        dmEmbed.setFooter({ text: 'Appeal this action in the server you were moderated in.' });
      }
    }
  ...
  try {
    ...
    if (action === 'note') {
      dmEmbed = new EmbedBuilder()
        ...
        .setFooter({ text: 'This is a verbal warning. No further action required.' })
        .setTimestamp();
    } else {
      dmEmbed = new EmbedBuilder()
        .setColor(0xFFFFFF)
        ...
        .setFooter({ text: 'Appeal this action in the server you were moderated in.' })
        .setTimestamp();
    }
    ...
    if (action === 'warn' || action === 'mute' || action === 'note') {
      const expirationDate = new Date();
      expirationDate.setDate(expirationDate.getDate() + 30);
      dmEmbed.addFields({ name: 'This action expires', value: `<t:${Math.floor(expirationDate.getTime() / 1000)}:R>`, inline: true });
    }
    await target.send({ embeds: [dmEmbed] });
  } catch (error) {
    console.error('Failed to send DM to moderated user:', error);
...
// ============================================================================
// Guild Approve Checkpoint 5/10
// ============================================================================
if (!isGuildApproved(guildId)) {
  return await message.reply('> This bot is currently not approved or activated.\n> You must request approval to activate this bot before use.\n \nSupport: https://discord.gg/PfCC7Y2tXH\nWebsite: https://sites.google.com/view/capitanfunny/discord-bot-developer?authuser=0');
}
...
case 'memberinfo': {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Joined Server', value: targetMember.joinedAt ? `<t:${Math.floor(targetMember.joinedAt.getTime() / 1000)}:F>` : 'Unknown', inline: true },
      { name: 'Avatar URL', value: `[Link](${user.displayAvatarURL({ dynamic: true, size: 1024 })})`, inline: false }
    )
    .setTimestamp();
...
case 'case':
  ...
  if (!amount || amount < 1 || amount > 100) {
    return await message.reply('<:a_2:1415171126560165928> Please provide a valid number of messages to delete (1-100).');
  }
  let targetUser = null;
  if (args[1]) {
...
if (!warnTarget) return await message.reply('<:a_2:1415171126560165928> Please provide a valid user (mention, ID, or username) to warn.');
        ...
        await message.reply(`<:y_:1377774946784251924> ${warnTarget.tag} has been warned. Case ID: \`${warnCaseId}\``);
        break;
...
if (!noteTarget) return await message.reply('<:a_2:1415171126560165928> Please provide a valid user (mention, ID, or username) to add a note for.');
  ...
  if (!member.permissions.has([PermissionsBitField.Flags.ManageMessages, PermissionsBitField.Flags.ModerateMembers])) {
    return await message.reply('<:a_2:1415171126560165928> You need the "Manage Messages" and "Timeout Members" permissions to use this command.');
  }
  ...
  try {
    ...
    if (error.code === 50013) {
      return await message.reply('<:a_2:1415171126560165928> I don\'t have permission to timeout this user. Check my role permissions and position in the role hierarchy.');
    }
    return await message.reply('<:a_2:1415171126560165928> Failed to timeout the user. Please try again.');
  }
      ...
      case 'about':
        const aboutEmbed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: 'Support Server', value: aboutInfo.supportServer, inline: true },
            { name: 'Website', value: aboutInfo.website, inline: true },
...
case 'commands': {
  ...
  break;
      ...
      case 'ping':
        ...
        await sent.edit(`Pong! Response time: ${latency}ms`);
        break;

      case 'status':
        const statusReply = await message.reply('Fully up and running!\n \n All systems operational.\n No issues/compromises detected.');
        setTimeout(() => {
...
case 'setproof': {
  ...
  if (option === 'dm.footer') {
    ...
    const footerText = args.slice(2).join(' ').trim();

    const validActions = ['warn', 'kick', 'ban', 'infract', 'all'];
    if (!action || !validActions.includes(action)) {
      return await message.reply('<:a_2:1415171126560165928> Usage: `!set dm.footer <warn|kick|ban|infract|all> <footer text>`');
    }
    if (!footerText) {
      return await message.reply('<:a_2:1415171126560165928> Please provide the footer text. Example: `!set dm.footer warn We currently do not have an appeal system!`');
    }

    try {
      if (action === 'all') {
        await setServerDmFooterPersist(guildId, 'warn', footerText);
        await setServerDmFooterPersist(guildId, 'kick', footerText);
        await setServerDmFooterPersist(guildId, 'ban', footerText);
        await setServerDmFooterPersist(guildId, 'infract', footerText);
      } else {
        await setServerDmFooterPersist(guildId, action, footerText);
      }
      return await message.reply(`<a:y1:1415173658237866025> DM footer updated for ${action === 'all' ? 'warn/kick/ban/infract' : action}.`);
    } catch (err) {
      console.error('Failed to set DM footer:', err);
      return await message.reply('<:a_2:1415171126560165928> Failed to save DM footer, check logs.');
    }
  }

  // ===== Existing Log Channel Logic =====
  const channel = message.mentions.channels.first();
  if (!option || !channel) {
    return await message.reply('<:a_2:1415171126560165928> Usage: `!set [modlogs/all.logs/verification.logs] #channel` or `!set dm.footer <warn|kick|ban|infract|all> <text>`');
  }

  if (!['modlogs', 'all.logs', 'verification.logs'].includes(option)) {
    return await message.reply('<:a_2:1415171126560165928> Log type must be either "modlogs", "all.logs", or "verification.logs".');
  }

  const logTypeForStorage = option === 'all.logs' ? 'alllogs' : option;
  await setServerLoggingChannelPersist(guildId, logTypeForStorage, channel.id);

  await message.reply(`<a:y1:1415173658237866025> ${option} will now be sent to ${channel}`);
  break;
...
serverinfo
setproof
- optimizations to !purge command; now can purge individual members
- intense optimization for all commands

## Coming soon;
- new "set" selection; moderation footer for dm's
- real and noticable optimizations
- updates in bot hoster; OCI (oracle cloud)
- public release (for anyone and everyone)
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  try {
    if (interaction.isStringSelectMenu() && interaction.customId === 'info_select') {
      ...
      const sel = INFO_SELECTIONS.find(s => s.id === chosen);
      if (!sel) {
        return await interaction.reply({ content: 'Unknown selection.', flags: 64 });
      }
      if (typeof sel.response === 'string') {
        await interaction.reply({ content: sel.response, flags: 64 });
      } else if (sel.response && sel.response.type === 'embed' && sel.response.embed) {
        await interaction.reply({ embeds: [sel.response.embed], flags: 64 });
      } else {
        await interaction.reply({ content: 'No content configured for this selection.', flags: 64 });
      }
      return;
    }
if (interaction.isButton() && interaction.customId.startsWith('verify_')) {
  ...
  if (!role) {
    ...
    return await interaction.editReply('<:a_2:1415171126560165928> The verified role no longer exists.');
  }
  if (!interaction.deferred && !interaction.replied) await interaction.deferReply({ flags: 64 });
  ...
  try {
    ...
    return await interaction.editReply('<:a_1:1415171153156374589> Failed to add the verified role. Please contact a moderator.');
  }
  try {
  ...
  return await interaction.editReply('<a:y1:1415173658237866025> You have been verified!');
    ...
    const guildId = guild.id;
    console.log('PSC:', guildId);
    if (!isGuildApproved(guildId)) {
      return await interaction.reply({ 
        content: '> This bot is currently not approved or activated.\n> You must request approval to activate this bot before use.\n \nSupport: https://discord.gg/PfCC7Y2tXH\nWebsite; https://sites.google.com/view/capitanfunny/discord-bot-developer?authuser=0',
        flags: 64 
      });
    }
  ...
  if (!verifiedRole) {
    return await interaction.reply({ content: '❌ You must specify a verified role.', flags: 64 });
  }
  ...
  await targetChannel.send({ embeds: [verifyEmbed], components: [verifyButton] });
  await interaction.reply({ content: `✅ Verification message sent in ${targetChannel}`, flags: 64 });
}


      case 'role': {
        if (!member.permissions.has(PermissionsBitField.Flags.ManageRoles)) {
          return await interaction.reply({ content: '<:a_2:1415171126560165928> You need the "Manage Roles" permission to use this command.', flags: 64 });
        }
        ...
        if (!action || !['add','remove'].includes(action)) {
          return await interaction.reply({ content: '<:a_2:1415171126560165928> Action must be either "add" or "remove"', flags: 64 });
        }

        if (!targetMember || !roleArg) {
          return await interaction.reply({ content: '<:a_2:1415171126560165928> Could not find the target user or role.', flags: 64 });
        }
        ...
        if (roleArg.position >= executorHighest.position) {
          return await interaction.reply({ content: '<:a_2:1415171126560165928> You cannot manage a role equal or higher than your highest role.', flags: 64 });
        }
        if (roleArg.position >= botHighest.position) {
          return await interaction.reply({ content: '<:a_2:1415171126560165928> I cannot manage that role because it is equal or higher than my highest role.', flags: 64 });
        }

        try {
          if (action === 'add') {
            if (targetMember.roles.cache.has(roleArg.id)) {
              return await interaction.reply({ content: '<:a_2:1415171126560165928> User already has that role.', flags: 64 });
            }
            await targetMember.roles.add(roleArg.id);
            await interaction.reply({ content: `<a:y1:1415173658237866025> Role ${roleArg.name} has been added to ${targetMember.user.tag}.`, flags: 64 });
          } else {
            if (!targetMember.roles.cache.has(roleArg.id)) {
              return await interaction.reply({ content: '<:a_2:1415171126560165928> User does not have that role.', flags: 64 });
            }
            await targetMember.roles.remove(roleArg.id);
            await interaction.reply({ content: `<a:y1:1415173658237866025> Role ${roleArg.name} has been removed from ${targetMember.user.tag}.`, flags: 64 });
          }
        } catch (err) {
          console.error('Role command error:', err);
          await interaction.reply({ content: '<:a_2:1415171126560165928> Failed to modify role. Check my permissions and role hierarchy.', flags: 64 });
        }
        break;
      }
      ...
      case 'say':
        ...
        await interaction.followUp(sayContent);
        break;

      case 'ping':
        await interaction.reply('Pong!');
        const latency = Date.now() - interaction.createdTimestamp;
        await interaction.editReply(`Pong! Response time: ${latency}ms`);
        break;
      ...
      case 'purge': {
        ...
        if (!amount || amount < 1 || amount > 100) {
          return await interaction.reply({ content: '<:a_2:1415171126560165928> Please provide a valid number of messages to delete (1-100).', flags: 64 });
        }

        try {
          if (!targetUser) {
            const deleted = await interaction.channel.bulkDelete(amount, true);
            await interaction.reply({ content: `<a:y1:1415173658237866025> Successfully deleted ${deleted.size} messages.`, flags: 64 });
          } else {
            const collected = [];
            let lastId = null;
            while (collected.length < amount) {
            ...
            if (collected.length === 0) {
              return await interaction.reply({ content: `<:a_2:1415171126560165928> No recent messages found for ${targetUser.tag}.`, flags: 64 });
            }

            const toDelete = collected.slice(0, 100);
            await interaction.channel.bulkDelete(toDelete, true);
            await interaction.reply({ content: `<a:y1:1415173658237866025> Deleted ${toDelete.length} message(s) from ${targetUser.tag}.`, flags: 64 });
          }
        } catch (err) {
          console.error('Purge error:', err);
          return await interaction.reply({ content: '<:a_2:1415171126560165928> An error occurred while trying to purge messages. Messages older than 14 days cannot be deleted in bulk.', flags: 64 });
        }
        break;
      }

      case 'status':
        await interaction.reply({ content: "Fully up and running!\n \n All systems operational.\n No issues/compromises detected.", flags: 64 });
        break;
      ...
      case 'mute':
        if (!member.permissions.has([PermissionsBitField.Flags.ManageMessages, PermissionsBitField.Flags.ModerateMembers])) {
          return await interaction.reply({ content: '<:a_2:1415171126560165928> You need the "Manage Messages" and "Timeout Members" permissions to use this command.', flags: 64 });
        }

        const muteTarget = interaction.options.getMember('user');
        ...
        await saveCasesToFile(guildId);
        await logModerationAction(guild, 'mute', interaction.user, muteTarget.user, muteReason, muteProof, muteCaseId, `${muteDuration} minutes`);
        await interaction.reply({ content: `<a:y1:1415173658237866025> ${muteTarget.user.tag} has been muted for ${muteDuration} minutes. Case ID: \`${muteCaseId}\``, flags: 64 });
        break;
  ...
  if (sub === 'logs') {
    const logType = interaction.options.getString('log_type', true)?.toLowerCase();
    ...
    await setServerLoggingChannelPersist(guildId, logTypeForStorage, channel.id);
    return await interaction.reply({ content: `<a:y1:1415173658237866025> ${logType} will now be sent to ${channel}`, flags: 64 });
  }
  ...
  if (sub === 'dmfooter') {
    ...
    try {
      if (action === 'all') {
        await setServerDmFooterPersist(guildId, 'warn', text);
        await setServerDmFooterPersist(guildId, 'kick', text);
        await setServerDmFooterPersist(guildId, 'ban', text);
        await setServerDmFooterPersist(guildId, 'infract', text);
      } else {
        await setServerDmFooterPersist(guildId, action, text);
      }
      return await interaction.reply({ content: `<a:y1:1415173658237866025> DM footer updated for ${action === 'all' ? 'warn/kick/ban/infract' : action}.`, flags: 64 });
    } catch (err) {
      console.error('Failed to save dm.footer via slash:', err);
      return await interaction.reply({ content: '<:a_2:1415171126560165928> Failed to save DM footer.', flags: 64 });
    }
  }

  return await interaction.reply({ content: '<:a_2:1415171126560165928> Unknown subcommand for /set', flags: 64 });
}
...
client.on(Events.ChannelUpdate, async (oldChannel, newChannel) => {
  ...
  if (changes.length > 0) {
    ...
    fields.push({ name: 'Changes', value: changes.join('\n'), inline: false });
...
new SlashCommandBuilder()
  ...
  .addSubcommand(sub =>
    sub.setName('logs')
      ...
      .addStringOption(opt => opt.setName('log_type').setDescription('The type of logs to configure')
        .setRequired(true)
        .addChoices(
  ...
  .addSubcommand(sub =>
    sub.setName('dmfooter')
      .setDescription('Set moderation DM footers (warn/kick/ban/infract)')
      .addStringOption(opt => opt.setName('action')
        .setDescription('Which DM footer to set (warn, kick, ban, infract, all)')
        .setRequired(true)
        .addChoices(
      ...
      .addStringOption(opt => opt.setName('text').setDescription('Footer text to add').setRequired(true))
  )
